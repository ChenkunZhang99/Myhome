...
export function HomeApp({ view = "overview" }: { view?: AppView }) {
  ...
  const scope = view === "inventory" ? "all" : "attention";
          ...
          <section className="content">
            ...
            <div className="dashboard" id="overview">
              {view === "overview" && (
                <>
                  ...
                  <section className="summary-grid" aria-label={t("库存摘要")}>
                    <a href="/inventory" className="summary-card green clickable">
                      ...
                      <div>
                        ...
                        <strong>
                          {loading
                            ...
                            : showingDemo
                              ? t("4 件示例")
                              : t("{count} 件", { count: items.length })}
                        </strong>
                        <small>{showingDemo ? t("等待真实录入") : t("查看全部分类库存")}</small>
                      </div>
                    </a>
                    <a href="/expiring" className="summary-card amber clickable">
                      <div className="summary-icon">
                        <Icon name="expiring" />
                      </div>
                      <div>
                    ...
                    <a href="/low-stock" className="summary-card coral clickable">
                    ...
                    <a href="/shopping" className="summary-card blue clickable">
                      <div className="summary-icon">$</div>
                      <div>
                        <span>{t("采购计划")}</span>
                        <strong>{t("已启用")}</strong>
              ...
              {view === "inventory" && (
                <section className="hero-row">
                  <div>
                    <p className="eyebrow">{t("家庭库存")}</p>
                    <h1>{t("按分类查看家里的物品")}</h1>
                    <p className="hero-copy">{t("这里是全部库存。临期和低库存仍会显示在总览里。")}</p>
                  </div>
                </section>
              )}
              ...
              {view === "flyers" && (
                <section className="hero-row">
                  <div>
                    ...
                    <h1>{t("Flyer 优惠")}</h1>
                    <p className="hero-copy">{t("收藏常去的超市，系统会同步它们的每周优惠。")}</p>
                  </div>
                </section>
              )}
                  ...
                  <div className="left-column">
                    <section className="panel inventory-panel" id="inventory">
                      <div className="section-head">
                        <div>
                          <p className="eyebrow">
                            {view === "inventory" ? t("全部库存") : t("需要关注")}
                          </p>
                          <h2>
                            {view === "inventory" ? t("分类库存") : t("临期与低库存")}
                            <small className="section-count">
                              {t("{count} 项", {
                                count:
                                  view === "inventory" ? filteredItems.length : attentionItems.length,
                              })}
                            </small>
                          </h2>
                        </div>
                        <div className="section-actions">
                          ...
                          {view === "overview" ? (
                            <a className="text-button" href="/inventory">
                              {t("查看全部库存")} <span>→</span>
                            </a>
                          ) : (
            ...
            <Modal eyebrow={t("真实库存")} title={t("添加一件物品")} onClose={() => setShowAdd(false)}>
              ...
              */}
              ...
              >
                <Icon name="camera" />
                <span>
                  <strong>{t("拍包装扫保质期")}</strong>
                  <small>{t("看不清会先给你几种可能，看清了再读日期。购买日期默认今天。")}</small>
                </span>
                <b aria-hidden="true">›</b>
              </button>
              </div>
              <form onSubmit={saveItem}>
                ...
                <div className="field-grid">
                  ...
                  <YmdDateInput prefix="purchase" label={t("购买日期")} value={dayIn(detectTimeZone())} />
