...
3. **会话管理** —— 改密码不撤销旧会话，用户也看不到其他登录设备或一键登出全部设备。
4. **HTTP 集成覆盖仍然很窄** —— 已有真实服务测试守住注册、会话和库存跨户隔离，但邀请、附件、注销、Flyer、菜谱和备份还没有走真实 HTTP。
5. **门店目录只进不出** —— 按邮编搜出来的店永远留着，错地址无处举报。
6. **PWA manifest** —— 手机不能加到主屏，而这是个「站在超市里用」的应用。
7. **发信服务未配** —— 没有 `RESEND_API_KEY`，忘记密码 = 永久锁死。界面已经如实说明并隐藏了那个入口。
8. **JS 渲染 + 不在 Flipp 的门店** 没有任何读取方式覆盖（目前实际数量为 0，先别投入）。
