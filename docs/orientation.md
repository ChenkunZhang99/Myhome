...
3. **集成测试** —— 见第二节盲区。开发期间写过几个一次性探针（多住户隔离、API 边界输入、共享额度），思路是「注册一个 `@e2e.test` 账号 → 打真实请求 → 断言 → 清理」，逻辑是对的但没进仓库，值得重写一遍固化进 CI。
4. **改密码不撤销旧会话** —— `setAccountPassword` 只清失败计数。
5. **安全响应头** —— 没有 CSP / HSTS / X-Frame-Options。
6. **门店目录只进不出** —— 按邮编搜出来的店永远留着，错地址无处举报。
7. **PWA manifest** —— 手机不能加到主屏，而这是个「站在超市里用」的应用。
8. **发信服务未配** —— 没有 `RESEND_API_KEY`，忘记密码 = 永久锁死。界面已经如实说明并隐藏了那个入口。
9. **JS 渲染 + 不在 Flipp 的门店** 没有任何读取方式覆盖（目前实际数量为 0，先别投入）。
